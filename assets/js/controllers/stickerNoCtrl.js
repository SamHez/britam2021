'use strict';
/** 
  * controller for User Sticker No Example
*/
...
app.controller('stickerNoCtrl', ["$scope",'$rootScope', '$http', '$filter', '$timeout','$stateParams','$state','$modal','$interval','toaster','SweetAlert','fileUpload','ngTableParams', function ($scope, $rootScope, $http, $filter, $timeout,$stateParams,$state,$modal,$interval,toaster,SweetAlert,fileUpload,ngTableParams) {
	...
	$scope.add_StickerNo = function(d)
	{ 
		
		var category = d.category;
		var sticker_no_from = d.sticker_no_from;
		var sticker_no_to = d.sticker_no_to;
		var total_amount_received = (sticker_no_to*1 - sticker_no_from*1); 
		
				if(typeof category ==='undefined')
				{
					category ='';
				}
				if(typeof sticker_no_from ==='undefined')
				{
					sticker_no_from ='';
				}
				if(typeof sticker_no_to ==='undefined')
				{
					sticker_no_to ='';
				}
				if(typeof total_amount_received ==='undefined')
				{
					total_amount_received =0;
				} 
				
		$http.post('api/?cmd=newStickerNo&category='+category+'&sticker_no_from='+sticker_no_from+'&sticker_no_to='+sticker_no_to+'&total_amount_received='+total_amount_received+'&user_id='+$rootScope.uid)
		.success(function(res)
		{
			...
			if(res.status=='ok')
			{
				toaster.pop('success', 'Sticker No Successful', res.message); 
			}
			else
			{
				toaster.pop('error', 'Sticker No Error', res.message);
			}
  ...
  $scope.deleteStickerNos=function(d)
		{
		...
		SweetAlert.swal({
            title: "Are you sure?",
            text: "Your will not be able to recover this Sticker No!",
            type: "warning",
            ...
            if (isConfirm) {
						...
						$http.post('api/?cmd=removeStickerNos&sticker_id='+ID)
						 .success(function(data)
						 {
							if(data.status=='ok')
								{  
								
					toaster.pop('success', "", 'Sticker Nos successfully removed', 10000, 'trustedHtml'); 	
					 
								}
							   else if(data.status=='empty' || data.status=='missing')
								   { 
					toaster.pop('error', "", 'Sticker Nos missing fields', 10000, 'trustedHtml'); 				
								   }
						 })
						.error(function(data,status)
										 {
											toaster.pop('error', "", 'Sticker Nos not removed', 10000, 'trustedHtml');  
						   }); 
	   
                SweetAlert.swal("Deleted!", "Sticker Nos has been deleted.", "success");
            } else {
                SweetAlert.swal("Cancelled", "Sticker Nos is safe :)", "error");
            }
        });
